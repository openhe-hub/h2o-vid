...
class HumanoidIm(humanoid_amp_task.HumanoidAMPTask):
    ...
    def _load_motion(self, motion_train_file, motion_test_file=[]):
        ...
        if self.smpl_humanoid:
            ...
            motion_eval_file = motion_train_file
            self._motion_train_lib = MotionLibH1(motion_file=motion_train_file, device=self.device, masterfoot_conifg=self._masterfoot_config, min_length=self._min_motion_len, im_eval=flags.im_eval)
            self._motion_eval_lib = MotionLibH1(motion_file=motion_eval_file, device=self.device, masterfoot_conifg=self._masterfoot_config, min_length=self._min_motion_len, im_eval=True)
